...
async def about_handler(callback: CallbackQuery):
    ...
    text = (
        f"{globe_emoji} <b>Stream Network VPN - stream net</b>\n\n"
        f"Stream Network VPN обеспечивает стабильный и безопасный доступ к сети. "
        ...
        f"Все серверы оптимизированы под высокую нагрузку, а соединение защищено сквозным "
        f"шифрованием и поддерживает обфускацию для обхода DPI.\n\n"
        f"Подключение и управление сервисом полностью автоматизировано через Telegram, "
        f"поэтому доступ к нему всегда остаётся простым и удобным, без ограничений со стороны "
        f"маркетплейсов или приложений."
    )
