...
// Прокси для OpenRouter (ключ на сервере)
app.post('/api/ai', async (req, res) => {
    ...
    try {
        ...
        if (!messages || !Array.isArray(messages)) {
            logError('❌ Некорректный формат запроса: messages отсутствует или не массив');
        ...
        if (!OPENROUTER_API_KEY) {
            logError('❌ OPENROUTER_API_KEY не найден в коде');
            return res.status(500).json({ error: 'API ключ не настроен на сервере' });
        }
        
        log('🔑 Ключ OpenRouter найден (прямо в коде)');
        ...
        logSuccess('✅ Ответ успешно отправлен клиенту');
